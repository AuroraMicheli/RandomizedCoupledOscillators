...
from utils import get_fordb_data, coRNN, coESN, check, LSTM
...
# LSTM:
# lr from 1e-4 to 1e-2, best 1e-2
# best test acc: 54% (for very few runs it goes up and matches hcornn)

# ESN:
# leaky in 0.001 0.01 0.1 0.5 1, best 0.001
# rho in 0.9 0.99, best 0.99
# inp in 0.1 1 10, best 1
# best test acc: 65%
...
train_loader, valid_loader, test_loader = get_fordb_data(args.batch, bs_test)
...
if args.esn:
    ...
    activations = scaler.transform(activations)
    classifier = LogisticRegression(max_iter=10000, penalty=None).fit(activations, ys)
    ...
    for epoch in range(args.epochs):
        ...
        if args.lstm:
            f = open(f'{main_folder}/fordb_log_lstm.txt', 'a')
        elif args.no_friction:
            f = open(f'{main_folder}/fordb_log_no_friction.txt', 'a')
        else:
            f = open(f'{main_folder}/fordb_log.txt', 'a')
...
if args.lstm:
    ...
    f = open(f'{main_folder}/fordb_log_coESN.txt', 'a')
elif args.esn: # ESN
    f = open(f'{main_folder}/fordb_log_esn.txt', 'a')
else: # original coRNN
    f = open(f'{main_folder}/fordb_log.txt', 'a')
ar = ''
