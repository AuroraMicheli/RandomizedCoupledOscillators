...
FILE=/tudelft.net/staff-bulk/ewi/insy/VisionLab/amicheli/RandomizedCoupledOscillators/spiking_ron.py
...
nohup apptainer exec --nv $CONTAINER python -u $FILE >> $RESULTSFOLDER/spiking_ron.log 2>&1 &

PID=$!
echo "Started job with PID: $PID"
echo $PID > "$RESULTSFOLDER/job.pid"
echo "tail -f $RESULTSFOLDER/spiking_ron.log"