...
class spiking_coESN_rescaled(nn.Module):
    ...
    READOUT STRATEGY: Time-Pooled Statistics (Mean + Std + Final State)
    ...
    def __init__(self, n_inp, n_hid, dt, gamma, epsilon, rho, input_scaling, 
                 theta_lif, theta_rf, tau_filter, count_lif_spikes=False, device='cpu', fading=False):
        super().__init__()
        self.n_hid = n_hid
        self.device = device
        self.fading = fading
    ...
    def forward(self, x):
        ...
        - hy_mean: temporal mean of HRF potential (rate coding)
        - hy_std: temporal standard deviation (variability/dynamics)
        - hy_final: final HRF state (sequence endpoint)
        """
        B = x.size(0)
        L = x.size(1)
        n_hid = self.n_hid
        ...
        hy_mean = hy_sum / L  # Average HRF trajectory
        hy_std = torch.sqrt(torch.clamp(hy_sq_sum / L - hy_mean ** 2, min=1e-8))  # Temporal variability
        hy_final = hy  # Final state
        
        # Concatenate features: 3*n_hid dimensional
        features = torch.cat([
            hy_mean,   # Mean activity over sequence
            hy_std,    # Variability captures dynamics
            hy_final   # Final state captures endpoint
        ], dim=1)
...
print(f"Feature extraction strategy: Mean + Std + Final State")
...
print(f"Feature dimensionality: {3 * args.n_hid} (3× for Mean/Std/Final)")
print(f"Validation accuracy: {valid_acc:.2f}%")
if test_feats is not None:
    print(f"Test accuracy: {test_acc:.2f}%")
