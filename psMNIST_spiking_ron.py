...
def extract_features(loader, model, device, perm):
    ...
    with torch.no_grad():
        for images, labels in tqdm(loader, ncols=80, desc="Extracting features"):
            ...
            # Reshape MNIST image (B, 1, 28, 28) -> sequential (B, 784, 1), then permute
...
def main():
    parser = argparse.ArgumentParser(description='Spiking RON on Permuted Sequential MNIST (psMNIST)')
    ...
    # Permutation seed (separate from model seed so permutation is fixed across trials)
    parser.add_argument('--perm_seed', type=int, default=0,
                        help="Seed for the fixed pixel permutation (default: 0). "
                             "Keep constant across runs to compare fairly.")
    ...
    print("=" * 70)
    print("SPIKING RON ON PERMUTED SEQUENTIAL MNIST (psMNIST)")
    print("=" * 70)
    print(args)
    print("=" * 70)
    ...
    n_inp      = 1    # psMNIST is pixel-by-pixel (univariate)
    ...
    # Generate fixed permutation once — same across all trials, controlled by perm_seed
    perm_rng = torch.Generator()
    perm_rng.manual_seed(args.perm_seed)
    perm = torch.randperm(784, generator=perm_rng).to(device)
    print(f"Fixed permutation generated with perm_seed={args.perm_seed}")

    print("\n=== Loading Permuted Sequential MNIST Dataset ===")
    train_loader, valid_loader, test_loader = get_mnist_data(args.batch, bs_test)
    print(f"Loaded psMNIST dataset")
    print(f"   Training samples: {len(train_loader.dataset)}")
    print(f"   Test samples:     {len(test_loader.dataset)}")
    print(f"   Sequence length:  {seq_length} (28x28 pixels, permuted pixel-by-pixel)")
    ...
    print(f"{'='*70}")
    print(f"Dataset:        Permuted Sequential MNIST (psMNIST)")
    print(f"Perm seed:      {args.perm_seed}")
    print(f"Hidden units:   {args.n_hid}, Trials: {args.test_trials}")
    print(f"Readout mode:   {args.readout_mode}")
    print(f"Sequence:       {seq_length} steps (permuted pixel-by-pixel)")
    ...
    results_filename = (
        f"results_psmnist_nhid{args.n_hid}_{conn_lif_str}_{conn_hrf_str}"
        f"_{args.readout_mode}"                                # NEW
        f"_perm{args.perm_seed}"
