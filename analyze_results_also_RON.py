...
def create_accuracy_vs_energy_plot(aggregated, n_hid, save_path=None):
    ...
    # Add RON datapoint for specific n_hid values
    ron_data = {
        256: {'energy': 2.373e-04, 'acc': 92.59, 'acc_std': 0.93},
        800: {'energy': 2.311e-03, 'acc': 95.21, 'acc_std': 0.58}
    }
    
    if n_hid in ron_data:
        ron_energy = ron_data[n_hid]['energy']
        ron_acc = ron_data[n_hid]['acc']
        ron_acc_std = ron_data[n_hid]['acc_std']
        
        ax.errorbar(ron_energy, ron_acc, 
                   yerr=ron_acc_std,
                   marker='o', markersize=10, 
        ...
        # Add text label for RON
        ax.annotate('RON', (ron_energy, ron_acc), 
                   textcoords="offset points", xytext=(10, 5),
...
def create_combined_plot(aggregated, save_path=None):
    ...
    for idx, n_hid in enumerate(n_hids):
        ...
        # Add RON datapoint if available for this n_hid
        if n_hid in ron_data:
            ron_energy = ron_data[n_hid]['energy']
            ron_acc = ron_data[n_hid]['acc']
            ron_acc_std = ron_data[n_hid]['acc_std']
